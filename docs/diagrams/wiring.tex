...
\begin{tikzpicture}[
    ...
    audio/.style    ={block, dashed, fill=orange!10, minimum width=42mm,
                      minimum height=14mm, font=\sffamily\footnotesize, align=left,
                      inner sep=3pt},
    spkr/.style     ={draw, thick, trapezium, trapezium left angle=70,
                      trapezium right angle=110, fill=gray!15, minimum width=10mm,
                      minimum height=8mm, font=\sffamily\scriptsize},
  ...
  % Audio modules: bottom-LEFT, below batt + L298N — full pinouts inline
  \node[audio, below=26mm of batt, xshift=2mm, anchor=north west] (mic)
        {\textbf{INMP441 I2S mic}\\[1pt]
         \scriptsize\begin{tabular}{@{}r@{\,}l@{}}
           VDD & $\leftarrow$ Pi pin 1 (3.3V)\\
           GND & $\leftarrow$ Pi GND\\
           L/R & $\leftarrow$ GND (= left)\\
           WS  & $\leftarrow$ GPIO 19 \textbf{\textcolor{red!75!black}{LRCLK}}\\
           SCK & $\leftarrow$ GPIO 18 BCLK\\
           SD  & $\rightarrow$ GPIO 20 DIN\\
         \end{tabular}};
  \node[audio, right=14mm of mic, anchor=north west] (amp)
        {\textbf{MAX98357A I2S amp}\\[1pt]
         \scriptsize\begin{tabular}{@{}r@{\,}l@{}}
           VIN  & $\leftarrow$ Pi pin 2 (5V)\\
           GND  & $\leftarrow$ Pi GND\\
           LRC  & $\leftarrow$ GPIO 19 \textbf{\textcolor{red!75!black}{LRCLK}}\\
           BCLK & $\leftarrow$ GPIO 18 BCLK\\
           DIN  & $\leftarrow$ GPIO 21 DOUT\\
           GAIN & open (default 9 dB)\\
           SD   & open (un-muted)\\
           +/$-$ & $\rightarrow$ speaker (4--8\,$\Omega$)\\
         \end{tabular}};

  % small speaker icon to the right of the amp
  \node[spkr, right=4mm of amp, yshift=-3mm] (spk) {spk};
  ...
  % branch down to mic — labels live inside the module node now, just route
  \draw[i2s] (busOverMic) -- (mic.north);
  % branch down to amp
  \draw[i2s] (busOverAmp) -- (amp.north);

  % speaker wires from amp's right side
  \draw[pwr, line width=0.5mm] ([yshift= 1mm]amp.east) -- ([yshift= 1mm]spk.west);
  \draw[gnd, line width=0.4mm] ([yshift=-1mm]amp.east) -- ([yshift=-1mm]spk.west);
  ...
  \node[font=\sffamily\footnotesize\itshape, text=black!55, align=center]
        at ($(mA)!0.5!(mB) + (0,10mm)$)
        {see \texttt{pi-pinout.pdf} for the full Pi GPIO header
         and recommended wire colours};
